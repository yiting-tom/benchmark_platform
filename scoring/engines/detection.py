...
class DetectionScoringEngine(BaseScoringEngine):
    ...
    def load_ground_truth(self) -> bool:
        ...
        if len(columns) < 6:
            self.log("Detection ground truth must have at least 6 columns: id, class, xmin, ymin, xmax, ymax", "ERROR")
            return False
        ...
        self.log(f"Auto-detected columns: ID='{self.id_col}', Class='{self.class_col}', BBox=[{self.xmin_col}, {self.ymin_col}, {self.xmax_col}, {self.ymax_col}]")
        
        # Prediction needs same columns + confidence
        self.REQUIRED_COLUMNS = [self.id_col, self.class_col, "confidence", 
                                  self.xmin_col, self.ymin_col, self.xmax_col, self.ymax_col]
    ...
    def validate_prediction_format(self, prediction_df: pd.DataFrame) -> tuple[bool, str]:
        ...
        if missing_columns:
            return False, f"Missing required columns: {missing_columns}. Expected: {self.REQUIRED_COLUMNS}"
        
        # Check for valid bounding box values
        for col in [self.xmin_col, self.ymin_col, self.xmax_col, self.ymax_col, "confidence"]:
            if not pd.api.types.is_numeric_dtype(prediction_df[col]):
                return False, f"Column '{col}' must be numeric"
        
        # Check confidence range
        if (prediction_df["confidence"] < 0).any() or (prediction_df["confidence"] > 1).any():
    ...
    def _calculate_ap_per_class(
        ...
        for image_id, group in gt_per_image:
            gt_boxes[image_id] = [
                (row[self.xmin_col], row[self.ymin_col], row[self.xmax_col], row[self.ymax_col])
        ...
        for _, pred in class_preds.iterrows():
            ...
            pred_box = (pred[self.xmin_col], pred[self.ymin_col], pred[self.xmax_col], pred[self.ymax_col])
