...
class SegmentationScoringEngine(BaseScoringEngine):
    ...
    def load_ground_truth(self) -> bool:
        ...
        if len(columns) < 5:
            self.log("Segmentation ground truth must have at least 5 columns: id, class, rle_mask, height, width", "ERROR")
            return False
        ...
        self.log(f"Auto-detected columns: ID='{self.id_col}', Class='{self.class_col}', RLE='{self.rle_col}', H='{self.height_col}', W='{self.width_col}'")
